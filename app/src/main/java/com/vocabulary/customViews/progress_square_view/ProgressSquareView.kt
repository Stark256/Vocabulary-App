...
class ProgressSquareView @JvmOverloads constructor(
    context: Context,
    ...
    private var squareSize: Float = context.resources.getDimension(R.dimen.progress_square_view_card_size)
    private var squareMargin: Float = context.resources.getDimension(R.dimen.progress_square_view_card_margin)
    ...
    var isTextLoadingVisible: Boolean
        get() { return textProgress?.visibility == View.VISIBLE }
        set(value) {  textProgress?.visibility = if(value) View.VISIBLE else View.GONE }

    init {
        val v = LayoutInflater.from(context).inflate(R.layout.view_progress_square, this, true)
    ...
    fun setPercentage(value: Int) {
        ...
        this.textProgress?.text = String.format(context.getString(R.string.loading), value.toString())
        if(percentage < 33) {
            val nextColor = Injector.themeManager.getGradient3(context)
            if(textProgress != null && textProgress!!.currentTextColor == nextColor) {
                animateChangingColor(textProgress!!, nextColor)
            }
        } else if(percentage < 66 && percentage > 33) {

            val nextColor = Injector.themeManager.getGradient2(context)
            if(textProgress != null && textProgress!!.currentTextColor == nextColor) {
                animateChangingColor(textProgress!!, nextColor)
            }
        } else if(percentage > 66) {
            val nextColor = Injector.themeManager.getGradient1(context)
            if(textProgress != null && textProgress!!.currentTextColor == nextColor) {
                animateChangingColor(textProgress!!, nextColor)
            }
        }
